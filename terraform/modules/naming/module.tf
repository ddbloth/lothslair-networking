...
resource "random_string" "numeric" {
  length  = 1
  special = false
  numeric = false
  upper   = false
...
locals {
  ...
  singlerandom_letter   = random_string.numeric.result
...
locals {
  ...
  filteredorgname = {
    alphanum    = replace(var.app_name, local.filter_alphanum, "")
    alphanumhup = replace(var.app_name, local.filter_alphanumhup, "")
    alphanumhu  = replace(var.app_name, local.filter_alphanumhu, "")
    alphanumh   = replace(var.app_name, local.filter_alphanumh, "")
  }
